...
public abstract class BufferedImageIterator<T> implements Iterator<T> {
    ...
    public static class Tile extends BufferedImageIterator<BufferedImage> {
        ...
        public Tile(BufferedImage image, int tileWidth, int tileHeight) {
            //The super class construtor calls the setImage method, but the
            //tile size needed for some calculations is not yet initialized
            super(image); 
            //Nevertheless, the call to setTileSize set all the variables
        ...
        public final void setTileSize(int tileWidth, int tileHeight) {
            ...
            dx = (int)tileWidth/2;
            dy = (int)tileHeight/2;
            if(source!=null){ //Updating variables depending on tile size
                in_width = Math.max(0, source.getWidth() - this.tileWidth) + 1;
                int in_height = Math.max(0, source.getHeight() - this.tileHeight) + 1;
